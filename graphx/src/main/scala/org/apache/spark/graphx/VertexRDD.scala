...
abstract class VertexRDD[VD](
  ...
  /**
    ...
    */
  override def compute(part: Partition, context: TaskContext): Iterator[DataElement[(VertexId, VD)]] = {
    val next = firstParent[ShippableVertexPartition[VD]].iterator(part, context).next()
    next.value.iterator.map(a => DataElement.of(a, task, task.isIgnored, next))
  }
...
object VertexRDD {
  ...
  def apply[VD: ClassTag](vertices: RDD[(VertexId, VD)]): VertexRDD[VD] = {
    ...
    val vertexPartitions = vPartitioned.mapPartitionsWithTaskInfo(
      { (iter, task) =>
        val cache = iter.toList
        Iterator(DataElement.of(ShippableVertexPartition(DataflowUtils.extractFromIterator(cache.toIterator)), task, task.isIgnored, cache: _*))
      },
  ...
  def apply[VD: ClassTag](
    ...
    val vertexPartitions = vPartitioned.zipPartitions(routingTables, preservesPartitioning = true) {
      (vertexIter, routingTableIter, task) =>
        val cache = vertexIter.toList
        ...
        Iterator(DataElement.of(
          ShippableVertexPartition(DataflowUtils.extractFromIterator(cache.toIterator), routingTable, defaultVal, mergeFunc),
          task, task.isIgnored, (dependency ++: cache): _*
        ))
    }
    new VertexRDDImpl(vertexPartitions)
  }
  ...
  def fromEdges[VD: ClassTag](
    ...
    val vertexPartitions = routingTables.mapPartitionsWithTaskInfo({ (routingTableIter, task) =>
      var dependency = Seq[DataElement[_]]()
      val routingTable = if (routingTableIter.hasNext) {
        val next = routingTableIter.next()
        dependency = Seq(next)
      ...
      Iterator(DataElement.of(
        ...
        task, task.isIgnored, dependency: _*
      ))
    }, preservesPartitioning = true)
    new VertexRDDImpl(vertexPartitions)
  }
  ...
  private[graphx] def createRoutingTables(
    ...
    val vid2pid = edges.partitionsRDD.mapPartitionsWithTaskInfo { (iter, task) =>
      iter.flatMap({ element =>
        val value = element.value
        val result = RoutingTablePartition.edgePartitionToMsgs(value._1, value._2)
        result.map(a => DataElement.of(a, task, task.isIgnored, element))
      })
    }.setName("VertexRDD.createRoutingTables - vid2pid (aggregation)")

    val numEdgePartitions = edges.partitions.length
    vid2pid.partitionBy(vertexPartitioner).mapPartitionsWithTaskInfo({
      (iter, task) =>
        val cache = iter.toList
        Iterator(DataElement.of(RoutingTablePartition.fromMsgs(numEdgePartitions, DataflowUtils.extractFromIterator(cache.toIterator)), task, task.isIgnored, cache: _*))
    }, preservesPartitioning = true)
  }
}
