...
abstract class RDD[T: ClassTag](
  ...
  def distinct(numPartitions: Int)(implicit ord: Ordering[T] = null): RDD[T] = withScope {
    def removeDuplicatesInPartition(partition: Iterator[DataElement[T]], taskOfMapPartition: Task): Iterator[DataElement[T]] = {
      ...
      map.insertAll(partition.map(data => (data.value, DataElement.of((data.value, null), task, true, data))))
      map.iterator.map(data => DataElement.of(data.value._1, taskOfMapPartition, taskOfMapPartition.isIgnored, data))
    }
    partitioner match {
      case Some(p) if numPartitions == partitions.length =>
        mapPartitionsWithTaskInfo(removeDuplicatesInPartition, preservesPartitioning = true).setTransformationType(TransformationType.DISTINCT)
      case _ =>  map(x => (x, null)).ignoreIt().reduceByKey((x, y) => x, numPartitions).ignoreIt().map(_._1).setTransformationType(TransformationType.DISTINCT)
    }
  }
    ...
    if (shuffle) {
      ...
      new CoalescedRDD(
        new ShuffledRDD[Int, T, T](
          mapPartitionsWithIndexInternalWithTaskInfo(distributePartition, isOrderSensitive = true).ignoreIt(),
