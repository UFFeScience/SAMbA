...
class AdaptiveSchedulingSuite extends SparkFunSuite with LocalSparkContext {
  ...
  test("fetching multiple map output partitions per reduce") {
    ...
    val dep = new ShuffleDependency[Int, Int, Int](rdd, new HashPartitioner(3), taskOfRDDWhichRequestThis = null)
  ...
  test("fetching all map output partitions in one reduce") {
    ...
    val dep = new ShuffleDependency[Int, Int, Int](rdd, new HashPartitioner(5), taskOfRDDWhichRequestThis = null)
